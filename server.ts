...
function generateCertificateCode(certType: string, courseTitleOrId: string, issueDate?: string | Date): string {
  const prefix = 'SV';
...
// ── CERTIFICATE VERIFICATION & MANAGEMENT ROUTES ──
...
app.post('/admin/lms/certificates/issue', requireAdmin, (req, res) => {
  ...
  let uId: number | null = user_id ? parseInt(user_id) : null;
